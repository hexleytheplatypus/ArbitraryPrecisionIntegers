...
public struct ArbitraryPrecisionUnsignedInteger: BinaryInteger {
    ...
    public init(integerLiteral value: StaticBigInt) {
        self.init(storage: value.words()) /// `_standardize()` called by `init<C: Sequence>(storage: C)`
    }

    public init<T: UnsignedInteger>(_ source: T) {
        guard source != T.zero else { self.init(); return }
        self.init(storage: source.words) /// `_standardize()` called by `init<C: Sequence>(storage: C)`
    }

    public init<T: BinaryInteger>(_ source: T) {
        // TODO: - Is it possible a SignedInteger creates problems here?
        guard source != T.zero else { self.init(); return }
        self.init(storage: source.words) /// `_standardize()` called by `init<C: Sequence>(storage: C)`
    }
...
import Memoize
extension ArbitraryPrecisionUnsignedInteger {
    fileprivate struct FactorialGenerator: Memoize {
        typealias Key = ArbitraryPrecisionUnsignedInteger
        typealias Value = ArbitraryPrecisionUnsignedInteger

        static var shared = Self()
        static var storage: Dictionary<Key, Value> = [:]

        static let memoized: ((Key) -> Value, Key) -> Value = { factorial, x in
            x == .zero ? 1 : x * factorial(x - 1)
        }

        init() { }

        static func of(_ key: Key) -> Value {
            return Self.shared.memoize(key)
        }
    }
